...
def voc_eval(attrpath,
    ...
    for imagename in imagenames:
        R = [attr for attr in recs[imagename] if attr['name'] == attr_name]
        attr = np.array([x['value'] for x in R])
        npos = npos + np.sum(attr)
        attr_recs[imagename] = {'attr': attr}
...
def roc_curve_single_attribute(attrpath,
    ...
    y_gt = np.array([attr_recs[imagename]['attr'][0] for imagename in imagenames])
...
def roc_curve_multi_attribute(attrpath,
    ...
    for i in range(n_attr):
        attr_recs = {}
        for imagename in imagenames:
            R = [attr for attr in recs[imagename] if attr['name'] == attr_name[i]]
            attr = np.array([x['value'] for x in R])
            attr_recs[imagename] = {'attr': attr}
        for j, imagename in enumerate(imagenames):
            y_gt[i, j] = attr_recs[imagename]['attr'][0]
...
def dataset_eval(annopath,
    ...
    if not os.path.isdir(cachedir):
        os.mkdir(cachedir)
    cachefile = os.path.join(cachedir, 'annots_trainval.pkl')
    # read list of images
    with open(imagesetfile, 'r') as f:
        lines = f.readlines()
    imagenames = [x.strip() for x in lines]
