...
class RoIDataLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        if cfg.TRAIN.HAS_RPN:
            ...
            # (n, x1, y1, x2, y2) specifying an image batch index n and a
            # rectangle (x1, y1, x2, y2)
            # this rois blob was shuffled randomly
            top[idx].reshape(cfg.TRAIN.BATCH_SIZE, 5)
            self._name_to_top_map['rois_shuffled'] = idx
            idx += 1

            # labels blob: R categorical labels in [0, ..., K] for K foreground
            # classes plus background
            # this labels blob was shuffled consistent with rois_shuffled
            top[idx].reshape(cfg.TRAIN.BATCH_SIZE)
            self._name_to_top_map['labels_shuffled'] = idx
            idx += 1
